...
/// Llamar justo después de SDL_GL_CreateContext, antes de cualquier draw call.
...
fn compilarShader(src: []const u8, tipo: c.GLenum) !c.GLuint {
    ...
    const ptrs               = [1][*c]const u8{ptr};
    const lens               = [1]c.GLint{@intCast(src.len)};
    glShaderSource(shader, 1, &ptrs, &lens);
    glCompileShader(shader);

    var status: c.GLint = 0;
    // FIX: *GLint no castea a [*]GLint en 0.14 sin @ptrCast explícito
    glGetShaderiv(shader, GL_COMPILE_STATUS, @as([*]c.GLint, @ptrCast(&status)));
    if (status == 0) {
        var log_buf: [1024]u8 = undefined;
        var log_len: c.GLsizei = 0;
        glGetShaderInfoLog(shader, 1024, @as(?[*]c.GLsizei, @ptrCast(&log_len)), &log_buf);
...
pub fn crearPrograma(vert_src: []const u8, frag_src: []const u8) !c.GLuint {
    ...
    var status: c.GLint = 0;
    // FIX: mismo ptrCast
    glGetProgramiv(prog, GL_LINK_STATUS, @as([*]c.GLint, @ptrCast(&status)));
    if (status == 0) {
        var log_buf: [1024]u8 = undefined;
        var log_len: c.GLsizei = 0;
        glGetProgramInfoLog(prog, 1024, @as(?[*]c.GLsizei, @ptrCast(&log_len)), &log_buf);
...
/// Crea una textura 1×1 de color sólido — útil como placeholder.
